    ...
    public class Iso15765NetwrorkLayer
    {
        ...
        public void Transport(byte[] request, out byte[] response)
        {
            ...
            do
            {
                ...
                catch (Iso15765TimeoutException e)
                {
                    ...
                    {
                        retryCount++;
                        Log.WriteLine("Retry, count:" + retryCount.ToString());
                    }
                    else
                    {
                        throw e;
        ...
        public void TransmittRequest(byte[] request, int flowControlTimeout)
        {
            ...
            if (length <= 7)
            {
                ...
                _link.Write(frame);
                Log.WriteLine("TransmittRequest.N_PCItypeSingleFrame->Done");
            }
            else
            {
                ...
                offset += 6;
                _link.Write(frame);
                Log.WriteLine("TransmittRequest.N_PCItypeFirstFrame->Read_N_PCItypeFlowControl.");

                //Read FlowControl
                frame = _link.Read(flowControlTimeout);
                Log.WriteLine("TransmittRequest.Read_N_PCItypeFlowControl->N_PCItypeConsecutiveFrame.");
                ...
                else
                {
                    ...
                    byte frameCnt = 0;
                    Log.WriteLine("BS:" + blockSize.ToString() + " frame, STmin:" + separationTime.ToString() + "ms.");
                    do
                    {
                        ...
                        if ((request.Length - offset) >= 7)
                        {
                            ...
                            offset += 7;
                            _link.Write(frame);
                            Log.WriteLine("TransmittRequest.N_PCItypeConsecutiveFrame->N_PCItypeConsecutiveFrame. Full Frame. New offset:" + offset);
                        }
                        else
                        {
                            ...
                            _link.Write(frame);
                            Log.WriteLine("TransmittRequest.N_PCItypeConsecutiveFrame->N_PCItypeConsecutiveFrame.");
                        }
                    ...
                    case N_PCItypeSingleFrame:
                        {
                            ...
                            Log.WriteLine("ReceiveResponse.N_PCItypeSingleFrame -> Done.");
                            isDone = true;
                            break;
                        }
                    case N_PCItypeFirstFrame:
                        {
                            ...
                            offset += 6;
                            Log.WriteLine("ReceiveResponse.N_PCItypeFirstFrame -> SendFlowControl.");
                            _link.Write(new byte[] { (N_PCItypeFlowControl), maxBlockSize, separationTime, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF });
                            Log.WriteLine("ReceiveResponse.N_PCItypeFirstFrame -> WaitFor:N_PCItypeConsecutiveFrame.");
                            isDone = false;
                            break;
                        }
                    case N_PCItypeConsecutiveFrame:
                        {
                            ...
                            {
                                isDone = true;
                                 Log.WriteLine("ReceiveResponse.N_PCItypeConsecutiveFrame -> Done");
                            }
                            else
                            {
                                if (maxBlockSize != 0)
                                {
                                    ...
                                    {  //Az adott blokkban már több frame nem lehet, most küldök egy FlowControl-t
                                        ...
                                        Log.WriteLine("ReceiveResponse.N_PCItypeFirstFrame -> SendFlowControl.");
                                        _link.Write(new byte[] { (N_PCItypeFlowControl), maxBlockSize, separationTime, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF });
                                        Log.WriteLine("ReceiveResponse.N_PCItypeConsecutiveFrame -> WaitFor:N_PCItypeConsecutiveFrame.");
                                    }
                                    else
                                    {
                                        isDone = false;
